...
// Last update Wed Jul  6 15:00:16 2016 
//

#include	<cstring>
#include	"autocompletion.hpp"

int		AutoCompletionTown::IsInCity(std::string town)
{
  unsigned int	i;
...
void		AutoCompletionTown::Init(std::vector<Town> town)
{
  unsigned int	i;
  unsigned int	j;
  ...
  while (i < town.size())
	...
	{
	  if (!IsInCity(town[i].city[j]))
	    {
	      City	new_city;

	      new_city.name = town[i].city[j];
	      new_city.size = 1;
	      getTown.push_back(new_city);
	    }
	  j = j + 1;
	}
      i = i + 1;
...
void		AutoCompletionTown::displayChoice(std::vector<char> choice)
{
  unsigned int	i;
...
int			AutoCompletionTown::checkEndWord()
{
  unsigned int			i;
...
std::vector<char>	AutoCompletionTown::getChoice()
{
  unsigned int			i;
  std::vector<char>		choice;
...
void			AutoCompletionTown::updateVector()
{
  unsigned int		i;
...
char				AutoCompletionTown::getInput(int word_end, int size)
{
  std::string			entry;
  int				getNbr;
...
bool				AutoCompletionTown::myCityCmp(Town t1, Town t2)
{
  t1 = t1;
  t2 = t2;
  // unsigned int			i;
  ...
  // i = 0;
  // while (i < t1.city.size() && i < t1.city.size())
  //   {
  //     if (strcmp(t1.city[i].c_str(), t2.city[i].c_str()) != 0)
  // 	return (strcmp(t1.city[i].c_str(), t2.city[i].c_str()));
  //   }
  // if (i == t1.city.size())
  //   return (false);
  // else
...
bool				AutoCompletionTown::cityExist(std::vector<Town> thisVector,
							  std::vector<std::string> town)
{
  ...
  while (i < thisVector.size())
    {
      if (vectorCmp(thisVector[i].city, town) == 0)
	return (true);
      i = i + 1;
    }
  return (false);
...
std::vector<Town>		AutoCompletionTown::removeUselessTown(std::vector<Town> town)
{
  unsigned int			i;
  ...
  while (i < town.size())
	...
	newTown.push_back(town[i]);
      else if (cityExist(newTown, town[i].city) == false)
	  newTown.push_back(town[i]);
      i = i + 1;
    }
  return (newTown);
...
void				AutoCompletionTown::displayFinalChoice(std::vector<std::vector<std::string> > & finalChoice)
{
  unsigned int			i;
  unsigned int			j;
  unsigned int			nbr;
...
std::vector<std::vector<std::string> >		AutoCompletionTown::getFinalChoice(std::vector<Town> town)
{
  unsigned int		i;
  unsigned int		j;
  ...
  while (i < town.size())
    {
      ...
      while (j < town[i].city.size())
	{
	  if (strcmp(town[i].city[j].c_str(), get.c_str()) == 0)
	    {
	      returnV.push_back(town[i].city);
	      break;
	    }
	  j = j + 1;
	}
...
std::vector<std::string>	AutoCompletionTown::autoGetTown(std::vector<Town> town)
{
  char				input;
  std::vector<char>		choice;
